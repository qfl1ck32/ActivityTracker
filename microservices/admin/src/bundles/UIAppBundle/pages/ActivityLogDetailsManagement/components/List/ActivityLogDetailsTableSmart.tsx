...
import { ActivityLogDetailList } from "../../config/ActivityLogDetailList";

export class ActivityLogDetailsAntTableSmart extends AntTableSmart<ActivityLogDetail> {
  collectionClass = ActivityLogDetailsCollection;

  getBody(): QueryBodyType<ActivityLogDetail> {
    return ActivityLogDetailList.getRequestBody();
  }

  getColumns(): ColumnsType<ActivityLogDetail> {
    const list = this.container.get(ActivityLogDetailList);
    list.build();

    return [...list.rest(), this.getActionsColumn()];
  }

  getActionsColumn(): ColumnType<ActivityLogDetail> {
    return {
  ...
  getSortMap() {
    return ActivityLogDetailList.getSortMap();
  }
  ...
  getActionItems() {
    ...
    if (features.view) {
      actions.push({
        ...
        icon: <EyeOutlined />,
        action: (model) => {
          this.router.go(Routes.ACTIVITY_LOG_DETAILS_VIEW, {
    ...
    if (features.edit) {
      actions.push({
        ...
        icon: <EditOutlined />,
        action: (model) => {
          this.router.go(Routes.ACTIVITY_LOG_DETAILS_EDIT, {
            params: { id: model._id.toString() },
          });
        },
      });
