...
import { ActivityNoteListFiltersForm } from "../../config/ActivityNoteListFiltersForm";

type ActivityNotesListFiltersProps = {
  onUpdate: (filters: any) => void;
};

export const ActivityNotesListFilters = React.memo(
  ActivityNotesListFiltersBase
);

export function ActivityNotesListFiltersBase(
  props: ActivityNotesListFiltersProps
) {
  ...
  const filterForm = use(ActivityNoteListFiltersForm, { transient: true });
  filterForm.build();
