...
import { ActivityTimingList } from "../../config/ActivityTimingList";

export class ActivityTimingsAntTableSmart extends AntTableSmart<ActivityTiming> {
  collectionClass = ActivityTimingsCollection;

  getBody(): QueryBodyType<ActivityTiming> {
    return ActivityTimingList.getRequestBody();
  }

  getColumns(): ColumnsType<ActivityTiming> {
    const list = this.container.get(ActivityTimingList);
    list.build();

    return [...list.rest(), this.getActionsColumn()];
  }

  getActionsColumn(): ColumnType<ActivityTiming> {
    return {
  ...
  getSortMap() {
    return ActivityTimingList.getSortMap();
  }
  ...
  getActionItems() {
    ...
    if (features.view) {
      actions.push({
        ...
        icon: <EyeOutlined />,
        action: (model) => {
          this.router.go(Routes.ACTIVITY_TIMINGS_VIEW, {
    ...
    if (features.edit) {
      actions.push({
        ...
        icon: <EditOutlined />,
        action: (model) => {
          this.router.go(Routes.ACTIVITY_TIMINGS_EDIT, {
            params: { id: model._id.toString() },
          });
        },
      });
