...
import { NoteModelList } from "../../config/NoteModelList";

export class NoteModelsAntTableSmart extends AntTableSmart<NoteModel> {
  collectionClass = NoteModelsCollection;

  getBody(): QueryBodyType<NoteModel> {
    return NoteModelList.getRequestBody();
  }

  getColumns(): ColumnsType<NoteModel> {
    const list = this.container.get(NoteModelList);
    list.build();

    return [...list.rest(), this.getActionsColumn()];
  }

  getActionsColumn(): ColumnType<NoteModel> {
    return {
  ...
  getSortMap() {
    return NoteModelList.getSortMap();
  }
  ...
  getActionItems() {
    ...
    if (features.view) {
      actions.push({
        ...
        icon: <EyeOutlined />,
        action: (model) => {
          this.router.go(Routes.NOTE_MODELS_VIEW, {
    ...
    if (features.edit) {
      actions.push({
        ...
        icon: <EditOutlined />,
        action: (model) => {
          this.router.go(Routes.NOTE_MODELS_EDIT, {
            params: { id: model._id.toString() },
          });
        },
      });
